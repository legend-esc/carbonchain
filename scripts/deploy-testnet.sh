...
# ── 5b. Emit environment variables for the API ─────────────────────────────
# The API reads contract IDs from environment variables.  Write a shell
# snippet so operators / CI can source them into their environment without
# manual copy-paste.
ENV_FILE="$SCRIPT_DIR/contract-env.sh"
cat > "$ENV_FILE" <<EOF
# Contract IDs — source this file to load into your environment.
# Generated by deploy-testnet.sh on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
export CREDIT_REGISTRY_CONTRACT_ID="$CREDIT_REGISTRY_ID"
export RETIREMENT_CONTRACT_ID="$RETIREMENT_ID"
export MARKETPLACE_CONTRACT_ID="$MARKETPLACE_ID"
export MRV_ORACLE_CONTRACT_ID="$MRV_ORACLE_ID"
export ADMIN_ADDRESS="$ADMIN_ADDRESS"
EOF
log "Contract env vars written to $ENV_FILE"
log "Source with: source $ENV_FILE"
