...
function run() {
  ...
  // Deterministic metadata so the generated bytes (and therefore the IPFS CID)
  // are reproducible for the same retirement. Without this pdfkit embeds the
  // current time, making certificate verification by content hash impossible.
  const fixedDate = new Date(data.timestamp * 1000);
  doc.info.CreationDate = fixedDate;
  doc.info.ModDate = fixedDate;
  const chunks = [];
